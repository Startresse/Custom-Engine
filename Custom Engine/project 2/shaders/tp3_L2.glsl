...
void main( )
{
    ...
    if (hit(ray, p, pn, tr, mat))
    {
        ...
        for (int i = 0; i < nb_ray_l2; ++i)
        {
            ...
            RayHit ray2 = RayHit(p, ray_p, -1, dist_max, 0.0, 0.0);
            ...
            if (!hit(ray2, q, qn, q_tr, q_mat))
                continue;

            vec3 L1 = L1_calc(x_rng, q, p, qn, q_mat);
            vec3 fr = fr_calc(q, p, ray.o, pn, mat);
            float cos_theta = max(0.0, dot(ray_p, pn));
            float inv_pdf = 1.0 / pdf35(random_ray);

            L2 = L2 + fr * L1 * cos_theta * inv_pdf;
        }
        L2 = L2 / float(nb_ray_l2);
    }

    imageStore(color_image, ivec3(xy, 2), vec4(L2, 1));
